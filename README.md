...
In our project, we conducted in-depth research on predicting the number of people in a room from various perspectives. We employed a range of preprocessing techniques and multiple machine learning models, including neural networks, logistic regression, decision trees, and support vector machines, to address this issue. Notably, we tackled the issue of category imbalance in the dataset and rectified it using random oversampling techniques. Additionally, we experimented with dimensionality reduction methods, such as principal component analysis (PCA), to investigate the significance of various characteristics.

We observed that different sensors have varying effects on room occupancy, potentially due to their placement. This discovery holds value for future research and practical applications. Through cross-validation and multiple experiments, we confirmed that our model did not suffer from overfitting and performed excellently in predicting the number of people in the room, achieving nearly 100% on the average score.
