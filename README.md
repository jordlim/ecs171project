...
---------------------------------------------------------------- (elson's part, can combine together with others)
## Data Exploration
...
To get different interpretations on our data, we chose to implement PCA and oversampling our data. 

Our data had around 17 attributes, so we decided to use PCA to reduce its dimensionality to 3 principal components, which helps us reduce complexity and allows to visualize the data more easily.
```
pca = PCA(n_components = 3)
reduced_x_train = pca.fit_transform(x_train_scaled)
reduced_x_test = pca.transform(x_test_scaled)
